...
USER_SERVICE_BASE_URL = os.getenv("USER_SERVICE_BASE_URL", "http://localhost:8001").rstrip("/")
...
def create_review(payload: ReviewCreate, db: Session = Depends(get_db)):
    ...
    try:
        db.commit()
        db.refresh(review)
        notif_url = f"{os.getenv('NOTIF_SERVICE_BASE_URL','http://localhost:8004').rstrip('/')}/api/notifications"
        payload_notif = {
